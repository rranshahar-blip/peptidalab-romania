...
const locale=['ro','en','pl','hu','bg','nl'].includes(document.documentElement.lang)?document.documentElement.lang:'ro',isEn=locale==='en',langRoot=locale==='ro'?'':`/${locale}`;
const uiByLocale={
ro:{added:'Adăugat ✓',add:'Adaugă în coș',cart:'Coș',cartTitle:'Coșul tău',emptyTitle:'Coșul tău este gol',emptyText:'Adaugă produse din catalog. Coșul rămâne salvat în acest browser.',explore:'Continuă cumpărăturile',continueShopping:'Continuă cumpărăturile',proceed:"Continuă cu cererea de comandă",remove:'Elimină',clear:'Golește coșul',subtotal:'Subtotal',totalProducts:'Total produse',unitPrice:'Preț unitar',lineTotal:'Total produs',quantity:'Cantitate',increase:'Mărește cantitatea',decrease:'Micșorează cantitatea',close:'Închide coșul',specialOffer:'Ofertă specială',promoPrice:'Preț promoțional',save:'Economisești 200 lei',noResults:'Niciun rezultat.',sending:'Se trimite…'},
en:{added:'Added ✓',add:'Add to cart',cart:'Cart',cartTitle:'Your cart',emptyTitle:'Your cart is empty',emptyText:'Add products from the catalogue. Your cart remains saved in this browser.',explore:'Continue shopping',continueShopping:'Continue shopping',proceed:"Continue to order request",remove:'Remove',clear:'Clear cart',subtotal:'Subtotal',totalProducts:'Total products',unitPrice:'Unit price',lineTotal:'Line total',quantity:'Quantity',increase:'Increase quantity',decrease:'Decrease quantity',close:'Close cart',specialOffer:'Special offer',promoPrice:'Promotional price',save:'You save 200 lei',noResults:'No results.',sending:'Sending…'},
pl:{added:'Dodano ✓',add:'Dodaj do koszyka',cart:'Koszyk',cartTitle:'Twój koszyk',emptyTitle:'Twój koszyk jest pusty',emptyText:'Dodaj produkty z katalogu. Koszyk zostanie zapisany w tej przeglądarce.',explore:'Kontynuuj zakupy',continueShopping:'Kontynuuj zakupy',proceed:"Przejdź do zapytania o zamówienie",remove:'Usuń',clear:'Wyczyść koszyk',subtotal:'Suma częściowa',totalProducts:'Łączna liczba produktów',unitPrice:'Cena jednostkowa',lineTotal:'Suma pozycji',quantity:'Ilość',increase:'Zwiększ ilość',decrease:'Zmniejsz ilość',close:'Zamknij koszyk',specialOffer:'Oferta specjalna',promoPrice:'Cena promocyjna',save:'Oszczędzasz 200 lei',noResults:'Brak wyników.',sending:'Wysyłanie…'},
hu:{added:'Hozzáadva ✓',add:'Kosárba',cart:'Kosár',cartTitle:'Az Ön kosara',emptyTitle:'A kosár üres',emptyText:'Adjon hozzá termékeket a katalógusból. A kosár ebben a böngészőben marad.',explore:'Vásárlás folytatása',continueShopping:'Vásárlás folytatása',proceed:"Tovább a megrendelési kéréshez",remove:'Eltávolítás',clear:'Kosár ürítése',subtotal:'Részösszeg',totalProducts:'Összes termék',unitPrice:'Egységár',lineTotal:'Tétel összege',quantity:'Mennyiség',increase:'Mennyiség növelése',decrease:'Mennyiség csökkentése',close:'Kosár bezárása',specialOffer:'Különleges ajánlat',promoPrice:'Akciós ár',save:'Megtakarítás: 200 lei',noResults:'Nincs találat.',sending:'Küldés…'},
bg:{added:'Добавено ✓',add:'Добавяне в количката',cart:'Количка',cartTitle:'Вашата количка',emptyTitle:'Количката ви е празна',emptyText:'Добавете продукти от каталога. Количката остава запазена в този браузър.',explore:'Продължете пазаруването',continueShopping:'Продължете пазаруването',proceed:"Продължи към заявката за поръчка",remove:'Премахване',clear:'Изчистване на количката',subtotal:'Междинна сума',totalProducts:'Общо продукти',unitPrice:'Единична цена',lineTotal:'Общо за артикула',quantity:'Количество',increase:'Увеличаване на количеството',decrease:'Намаляване на количеството',close:'Затваряне на количката',specialOffer:'Специална оферта',promoPrice:'Промоционална цена',save:'Спестявате 200 lei',noResults:'Няма резултати.',sending:'Изпращане…'},
nl:{added:'Toegevoegd ✓',add:'Toevoegen aan winkelwagen',cart:'Winkelwagen',cartTitle:'Uw winkelwagen',emptyTitle:'Uw winkelwagen is leeg',emptyText:'Voeg producten uit de catalogus toe. Uw winkelwagen blijft in deze browser opgeslagen.',explore:'Verder winkelen',continueShopping:'Verder winkelen',proceed:"Doorgaan naar bestelaanvraag",remove:'Verwijderen',clear:'Winkelwagen legen',subtotal:'Subtotaal',totalProducts:'Totaal aantal producten',unitPrice:'Eenheidsprijs',lineTotal:'Regeltotaal',quantity:'Aantal',increase:'Aantal verhogen',decrease:'Aantal verlagen',close:'Winkelwagen sluiten',specialOffer:'Speciale aanbieding',promoPrice:'Actieprijs',save:'U bespaart 200 lei',noResults:'Geen resultaten.',sending:'Verzenden…'}
};
const ui=uiByLocale[locale];
$('.language-select')?.addEventListener('change',event=>{storage.set('pl_lang',event.target.selectedOptions[0]?.lang||locale);window.location.assign(event.target.value)});
...
function show(el){if(el){dialogTriggers.set(el,document.activeElement);el.hidden=false;setBodyLock();requestAnimationFrame(()=>$('button:not([disabled]),input:not([type="hidden"]),select,a[href]',el)?.focus())}}
function hide(el){if(el){const wasOpen=!el.hidden;el.hidden=true;setBodyLock();if(wasOpen)dialogTriggers.get(el)?.focus?.()}}
...
function syncClarity(allowed){
  ...
  if(!window.__plClarityLoaded){
    ...
    document.head.appendChild(script);window.__plClarityLoaded=true;
  }
}
function syncTrackingConsent(allowed){syncMetaPixel(allowed);syncClarity(allowed)}
...
syncTrackingConsent(hasMetaConsent());
...
window.addEventListener('storage',e=>{if(e.key===null||e.key==='pl_cookie'||e.key==='pl_meta_consent_version')syncTrackingConsent(hasMetaConsent())});
...
const numberLocale={ro:'ro-RO',en:'en-GB',pl:'pl-PL',hu:'hu-HU',bg:'bg-BG',nl:'nl-NL'}[locale];
const money=value=>`${Number(value).toLocaleString(numberLocale)} lei`;
const productName=product=>locale!=='ro'&&product.nameEn?product.nameEn:product.name,productConcentration=product=>locale!=='ro'&&product.concentrationEn?product.concentrationEn:product.concentration;
...
function fillQuote(){const q=$('#quote-products');if(!q)return;const next=cart.length?quoteSummary():'';if(!q.value.trim()||q.value===lastQuoteSummary){q.value=next;lastQuoteSummary=next}}
...
function filterCatalog(){if(!grid)return;const q=(catalogSearch?.value||'').trim().toLocaleLowerCase(numberLocale),cat=catFilter?.value||'all',conc=concFilter?.value||'all';let cards=$$('.product-card',grid);cards.forEach(c=>{const yes=(!q||(c.dataset.name+' '+c.dataset.concentration).toLocaleLowerCase(numberLocale).includes(q))&&(cat==='all'||c.dataset.category===cat)&&(conc==='all'||c.dataset.concentration.includes(conc));c.hidden=!yes});cards=cards.filter(c=>!c.hidden);cards.sort((a,b)=>a.dataset.name.localeCompare(b.dataset.name,numberLocale)*(sortFilter?.value==='za'?-1:1)).forEach(c=>grid.appendChild(c));if($('#catalog-count'))$('#catalog-count').textContent=cards.length;if($('#no-results'))$('#no-results').hidden=!!cards.length}
